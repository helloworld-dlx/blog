...
- MajorPath 当前允许公开项目名与“公益专业认知与探索工具”的高层定位；公开 GitHub 链接需用户明确批准。
- 一生一芯相关代码仓库不在博客中公开链接。
- 课程项目可以没有 GitHub 仓库；有真实截图、报告或其它证据即可作为 Portfolio 项目。
...
Changelog 是给读者看的站点演进记录，不是 Git log。

### 什么时候记录

适合记录：

- `site` — 全站 UI、导航、栏目、信息架构或重要页面能力发生明显变化。
- `content` — 新增重要内容体系、作品集结构或其它读者能明显感知的内容级变化。
- `deploy` — 域名、托管、CDN、评论系统等部署方式发生变化。
- `fix` — 真正影响访问、阅读、构建或主要功能的 Bug 修复。

默认不记录：

- 新建草稿或普通文章发布；
- `pinned` / `featured` 调整；
- tag、metadata、错别字或小文案修改；
- 普通依赖升级；
- 纯内部重构且读者无感知。

### 执行规则

1. 用户明确说“记到更新日志 / 更新日志”时，追加 `src/data/changelog.ts`。
2. 如果刚完成的是明显的站点级变化，可以**建议一次**是否记录，但不能未经同意自动写入。
3. date 未指定时使用当天日期。
4. 只记录真实已经发生的变更；历史条目保持当时的名称和语境，不因后来改名而回写历史。
5. 描述应简短、面向读者，不列内部 commit、hash 或测试细节。
6. 实际写入后运行 `npm run build`。
7. 不 commit，不 push。
